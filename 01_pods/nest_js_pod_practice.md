...
Spring Boot에 이어, Node.js 기반의 프레임워크인 **Nest.js** 애플리케이션을 파드로 띄워보는 실습을 진행합니다.

## 1. 실습 준비

### ① Nest.js 프로젝트 생성
로컬 환경에 Nest.js CLI가 설치되어 있어야 합니다. (이미 프로젝트가 있다면 이 단계를 건너뛰어도 됩니다.)
...
Nest.js 프로젝트 루트 디렉토리에 `Dockerfile`과 `.dockerignore` 파일을 작성합니다.
...
호스트의 3000 포트를 파드의 3000 포트로 연결합니다.
```bash
$ kubectl port-forward pod/nest-pod 3000:3000
```
브라우저에서 `http://localhost:3000`에 접속하여 "Hello World!"(또는 설정한 응답)가 나오는지 확인합니다.

### ② 파드 내부 접속 (kubectl exec)
파드 내부에서 직접 응답을 확인해 볼 수도 있습니다.
```bash
...
각 프레임워크마다 사용하는 포트와 환경이 다르지만, 쿠버네티스에서는 동일한 **파드(Pod)**라는 단위로 추상화되어 관리됨을 알 수 있습니다.
