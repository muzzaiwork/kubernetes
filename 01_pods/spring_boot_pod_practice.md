...
직접 작성한 코드를 Docker 이미지로 만들어서 쿠버네티스 파드로 띄워보는 전체 과정을 실습합니다.

### ① 프로젝트 준비 및 빌드
실습을 위해 준비된 Spring Boot 프로젝트를 빌드하여 JAR 파일을 생성합니다.
...
### ② Docker 이미지 빌드
Dockerfile을 바탕으로 `spring-server`라는 이름의 이미지를 로컬에 생성합니다.
...
이미지 태그를 명시하지 않으면 쿠버네티스는 기본적으로 `latest` 태그를 사용하며, 이미지 풀 정책을 **`Always`**로 설정합니다. 이 경우 쿠버네티스는 로컬에 이미지가 있더라도 무조건 원격 레지스트리(Docker Hub 등)에서 이미지를 찾으려 시도합니다. `spring-server`는 로컬에서만 만든 이미지이므로 원격에서 찾지 못해 에러가 발생하는 것입니다.

- 상세 설명: [이미지 풀 정책(ImagePullPolicy) 이해하기](../01_concepts/image_pull_policy.md)

### ④ 문제 해결: 매니페스트 수정 및 재배포
로컬 이미지를 우선적으로 사용하도록 `imagePullPolicy: IfNotPresent`를 추가합니다.
...
서버가 정상적으로 기동되었다면 아래 방법들로 응답을 확인할 수 있습니다.
