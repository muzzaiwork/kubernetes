...
<aside>
🧑🏻‍🏫 **교안**  
실제 서비스를 운영하다 보면 트래픽이 증가해서 서버가 버벅거리는 경우가 생깁니다. 이때는 서버를 수평적 확장(서버의 개수를 늘리는 방식)을 통해 해결합니다. 이런 상황을 가정해 백엔드 서버인 Spring Boot 서버를 3대로 늘려보겠습니다.
</aside>
...
이전 실습에서는 파드 3개를 띄우기 위해 각각의 파드를 정의해야 했습니다. 하지만 디플로이먼트를 사용하면 하나의 설정으로 여러 개의 파드를 관리할 수 있습니다.

#### ❌ 기존 방식 (spring-pod-triple.yaml)
각각의 파드 이름을 다르게 지정하여 3번 정의해야 했습니다.
```yaml
apiVersion: v1
kind: Pod
metadata:
...
`replicas: 3` 설정 하나로 동일한 파드 3개를 생성합니다.
```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
...
새로운 방식으로 배포하기 전에, 이전에 개별적으로 생성했던 파드들을 삭제합니다.
```bash
...
디플로이먼트가 생성되면 자동으로 레플리카셋과 파드가 생성됩니다.
...
백엔드 서버 3개를 각각의 파드에 띄웠습니다. 하지만 실제 서비스 시에는 사용자 요청을 이 3개의 서버에 골고루 나누어주어야 합니다.

> **다음 단계**: 파드 앞단에서 트래픽을 균등하게 분배해주는 **서비스(Service)**에 대해 알아보겠습니다.
